...
internal static class PlayerGraphicsHooks
{
    ...
    private static void PlayerGraphics_AddToContainer(On.PlayerGraphics.orig_AddToContainer orig, PlayerGraphics self, RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam, FContainer newContatiner)
    {
        ...
        // Ensure both containers are available before proceeding.
        if (midground == null || foreground == null)
        {
            Plugin.DebugError("Midground or Foreground container is null in PlayerGraphics_AddToContainer");
            return;
        }

        // Ensure the sprite array exists and is large enough.
        if (sLeaser.sprites != null && sLeaser.sprites.Length >= slugTufts + 13)
        {
            for (int i = 0; i < slugTufts; i++)
            {
                int targetIndex = i + 13;
                if (targetIndex >= sLeaser.sprites.Length || sLeaser.sprites[targetIndex] == null)
                    continue;

                // Remove the sprite from the foreground container and add it to the midground container.
                foreground.RemoveChild(sLeaser.sprites[targetIndex]);
                midground.AddChild(sLeaser.sprites[targetIndex]);

                // Parse the secondary index safely.
                if (chunks[i].Length > 1 && int.TryParse(chunks[i][1].ToString(), out int otherIndex)
                    && otherIndex < sLeaser.sprites.Length && sLeaser.sprites[otherIndex] != null)
                {
                    sLeaser.sprites[targetIndex].MoveBehindOtherNode(sLeaser.sprites[otherIndex]);
                }
            }
        }
    }
