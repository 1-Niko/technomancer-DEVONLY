        ...
        Pass
        {
            ...
            {   
                ...
                for (uint j = 0; j < point_count; j++) // The point loop
                {
                    ...
                    if (lerpAB.x > _Cutoff)
                    {
                        // If flickering is disabled, bypass the flickering check.
                        if (modifiedDistance(float2(colour.r, colour.g), lerpAB, float2(_Width, _Height)) < _Size &&
                           (_FlickeringDisabled != 0 || (scanline(float2(colour.r, colour.g), _Offset / 67) + random(float2(i.uv.x, i.uv.y + _RandomOffset)) < -0.8)))
                        {
